...
async def smart_rerank_results(query: str, documents: List) -> tuple:
    ...
    if len(documents) <= 2:
        ...
        if all_decent_quality:
            logger.info(f"✅ Smart: Skipping LLM - only {len(documents)} candidates with decent quality (>= 0.70)")
            ...
            logger.info(f"🤖 Smart: Using LLM despite few candidates - low quality scores detected")
...
async def execute_search(system_components: Dict, query: str, rerank_mode: str = "smart"):
    ...
    try:
        ...
        logger.info(f"✓ Generated {len(rewrite_result.rewrites)} variants | Method: {rewrite_result.method} | Time: {rewrite_time:.3f}s")
        ...
        logger.info(f"✓ Retrieved {len(multi_retrieval_result.results)} candidates | Methods: {', '.join(multi_retrieval_result.methods_used)} | Time: {retrieval_time:.3f}s")
        ...
        if quality_results:
            logger.info(f"✓ {len(quality_results)} documents passed quality threshold ({MIN_RELEVANCE_THRESHOLD})")
        else:
            best_score = max([r.similarity_score for r in fusion_result.fused_results]) if fusion_result.fused_results else 0
            logger.info(f"✗ No documents passed quality threshold (best: {best_score:.3f}, threshold: {MIN_RELEVANCE_THRESHOLD})")
        ...
        logger.info(f"✓ Answer generated | Length: {len(answer)} chars | Time: {answer_time:.3f}s")
